...
export const test: Test = async ({ Command, expect, FileSystem, Locator, Workspace }) => {
  ...
  await expect(error).toContainText('GitHub returned an invalid pull request file list.')
  await expect(error).toContainText('Error code: E_GITHUB_INVALID_FILE_DATA')
}
