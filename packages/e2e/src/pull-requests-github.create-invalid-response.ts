...
export const name = 'pull-requests-github.create-invalid-response'
export const test: Test = async ({ Command, expect, FileSystem, Locator, Workspace }) => {
  ...
  const fixture = { defaults, responses: [{ body: { number: 42, url: 'https://evil.example' } }] }
  ...
  await retry(() => expect(element1).toContainText('Invalid pull request response'))
  const element2 = Locator('.PullRequestCreatedLink')
  await retry(() => expect(element2).toBeHidden())
}
