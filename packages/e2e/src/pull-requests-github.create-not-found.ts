...
export const name = 'pull-requests-github.create-not-found'
export const test: Test = async ({ Command, expect, FileSystem, Locator, Workspace }) => {
  ...
  const fixture = { defaults, responses: [{ body: { error: 'Repository not found' }, status: 404 }] }
  ...
  await retry(() => expect(element1).toContainText('Repository not found'))
