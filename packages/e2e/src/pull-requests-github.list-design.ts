...
`,
  ...
  const refreshButton = Locator('button[name="refreshPullRequests"]')
  const search = Locator('.PullRequestSearch')
  const searchInput = Locator('input[name="filterPullRequests"]')
  const stateIcons = Locator('.PullRequestStateIcon')
  const titles = Locator('.PullRequestListItemTitle')
  const repositoryContext = Locator('.PullRequestDescription')
  const duplicateHeading = Locator('.PullRequestIntro .PullRequestTitle')
  const secondTitle = titles.nth(1)
  const openTabCount = openTab.locator('.PullRequestTabCount')
  await expect(openTabCount).toContainText('3')
  const closedTabCount = closedTab.locator('.PullRequestTabCount')
  await expect(closedTabCount).toContainText('2')
  await expect(duplicateHeading).toHaveCount(0)
  await expect(repositoryContext).toContainText('lvce-editor / pull-request-github')
  await expect(searchInput).toBeVisible()
  await expect(createButton).toBeVisible()
  await expect(refreshButton).toBeVisible()
  await expect(items).toHaveCount(3)
  await expect(listCard).toHaveCSS('border-radius', '6px')
  await expect(search).toHaveCSS('border-radius', '4px')
  await expect(stateIcons).toHaveCount(3)
  await expect(stateIcons.first()).toHaveCSS('display', 'block')
