...
export const name = 'pull-requests-github.create-duplicate'
export const test: Test = async ({ Command, expect, FileSystem, Locator, Workspace }) => {
  ...
  const fixture = { defaults, responses: [{ body: { error: 'A pull request already exists' }, status: 422 }] }
  ...
  await retry(() => expect(element1).toContainText('A pull request already exists'))
