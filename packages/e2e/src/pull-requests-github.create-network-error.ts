...
export const name = 'pull-requests-github.create-network-error'
export const test: Test = async ({ Command, expect, FileSystem, Locator, Workspace }) => {
  ...
  const fixture = { defaults, responses: [{ error: 'offline' }] }
  ...
  await retry(() => expect(element1).toContainText('Check the repository for an existing pull request'))
}
