...
export const name = 'pull-requests-github.create-rate-limit'
export const test: Test = async ({ Command, expect, FileSystem, Locator, Workspace }) => {
  ...
  const fixture = { defaults, responses: [{ body: { error: 'API rate limit exceeded' }, status: 429 }] }
  ...
  await retry(() => expect(element1).toContainText('API rate limit exceeded'))
