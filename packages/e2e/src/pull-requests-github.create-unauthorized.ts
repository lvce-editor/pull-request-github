...
export const name = 'pull-requests-github.create-unauthorized'
export const test: Test = async ({ Command, expect, FileSystem, Locator, Workspace }) => {
  ...
  const fixture = { defaults, responses: [{ body: { error: 'Session expired' }, status: 401 }] }
  ...
  await retry(() => expect(element1).toContainText('Session expired'))
