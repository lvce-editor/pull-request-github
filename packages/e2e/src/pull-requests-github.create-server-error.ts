...
export const name = 'pull-requests-github.create-server-error'
export const test: Test = async ({ Command, expect, FileSystem, Locator, Workspace }) => {
  ...
  const fixture = { defaults, responses: [{ body: { error: 'GitHub unavailable' }, status: 500 }] }
  ...
  await retry(() => expect(element1).toContainText('GitHub unavailable'))
